import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { hasInjectionContext, inject, toRef, computed, useSlots, ref, mergeProps, unref, withCtx, renderSlot, openBlock, createBlock, createCommentVNode, toDisplayString, createVNode, defineComponent, h, isRef, shallowRef, getCurrentInstance, provide, cloneVNode, createElementBlock, resolveComponent, toValue, resolveDynamicComponent, watch, nextTick, onServerPrefetch, useModel, createTextVNode, mergeModels, reactive, defineAsyncComponent, useSSRContext, shallowReactive, Suspense, Fragment, createApp, renderList, useId, onErrorCaptured, effectScope, useTemplateRef, withModifiers, withAsyncContext, getCurrentScope, markRaw, isReadonly, toRaw, isShallow, isReactive, customRef } from 'vue';
import { l as serialize, m as klona, n as defu, o as hasProtocol, q as isScriptProtocol, k as joinURL, r as parseQuery, v as defuFn, w as withQuery, x as sanitizeStatusCode, y as parseURL, z as encodePath, A as decodePath, B as isEqual, C as getContext, D as withTrailingSlash, E as withoutTrailingSlash, $ as $fetch$1, F as baseURL, G as hash, i as createError$1, H as executeAsync, I as getRequestHeader, J as setCookie, K as getCookie, L as deleteCookie } from '../_/nitro.mjs';
import { defineStore, getActivePinia, setActivePinia, createPinia, shouldHydrate } from 'pinia';
import { useRoute as useRoute$1, RouterView, createMemoryHistory, createRouter, START_LOCATION } from 'vue-router';
import { isPlainObject } from '@vue/shared';
import { Icon, getIcon, loadIcon as loadIcon$1, _api, addAPIProvider, setCustomIconsLoader } from '@iconify/vue';
import colors from 'tailwindcss/colors';
import { ssrRenderComponent, ssrRenderSlot, ssrRenderClass, ssrInterpolate, ssrRenderVNode, ssrRenderList, ssrRenderSuspense, ssrRenderAttrs, ssrRenderStyle } from 'vue/server-renderer';
import { useForwardProps, createContext, Primitive, Slot, ToastProvider, ToastPortal, ToastViewport, ConfigProvider, TooltipProvider, useForwardPropsEmits, ToastRoot, ToastTitle, ToastDescription, ToastAction, ToastClose, ProgressRoot, ProgressIndicator } from 'reka-ui';
...
const _routes = [
  {
    name: "Products-slug",
    path: "/Products/:slug()",
    component: () => import('./_slug_-BUkXy_md.mjs')
  },
  {
    name: "Products",
    path: "/Products",
    component: () => import('./index-Chcc27Rb.mjs')
  },
  {
    name: "slug",
    path: "/:slug()",
    component: () => import('./_slug_-CObeEKj2.mjs')
  },
  {
    name: "index",
    path: "/",
    component: () => import('./index-BRREM8fu.mjs')
  }
];
...
const namedMiddleware = {
  auth: () => import('./auth-Did7tuOt.mjs'),
  guest: () => import('./guest-D_cVVuPL.mjs')
};
...
const useAuthStore = defineStore("auth", {
  ...
  actions: {
    ...
    async userInfo() {
      try {
        const { $api } = useNuxtApp();
        const response = await $api("/customer/user");
        this.user = response.user;
      } catch (error) {
        throw error;
      }
...
const layouts = {
  default: defineAsyncComponent(() => import('./default-CktlkY-y.mjs').then((m) => m.default || m)),
  guest: defineAsyncComponent(() => import('./guest-DWRuIPWw.mjs').then((m) => m.default || m))
};
...
const useCartStore = defineStore("cart", {
  ...
  actions: {
    ...
    getProductIdentifier(product) {
      return product.sku ? `${product.sku}-${product.variant ?? ""}` : String(product.id);
    },
    addToCart(product) {
  ...
  getters: {
    cartCount: (state) => state.cart.reduce((acc, item) => acc + (item.quantity || 0), 0),
    cartTotal: (state) => state.cart.reduce(
      (acc, item) => acc + (item.price || 0) * (item.quantity || 1),
      0
    ),
    getCartTotalPrice: (state) => state.cart.reduce(
      (acc, item) => acc + (item.price || 0) * (item.quantity || 1),
      0
    ),
...
const useWishlistStore = defineStore("wishlist", {
  ...
  actions: {
    ...
    removeFromWishlist(item) {
      const productIdentifier = this.getProductIdentifier(item);
      const index2 = this.wishlist.findIndex(
        (w) => this.getProductIdentifier(w) === productIdentifier
      );
      if (index2 > -1) {
        const removed = this.wishlist[index2];
    ...
    moveToCart(item, cartStore) {
      const productIdentifier = this.getProductIdentifier(item);
      const index2 = this.wishlist.findIndex(
        (w) => this.getProductIdentifier(w) === productIdentifier
      );
      if (index2 > -1) {
        const moved = this.wishlist[index2];
        this.wishlist.splice(index2, 1);
        this.saveWishlist();
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-D7S57rDN.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-CxEbMIEK.mjs'));
...
export { __nuxt_component_0$1 as _, useRoute as a, useLocale as b, useAppConfig as c, useComponentUI as d, entry_default as default, usePortal as e, _sfc_main$8 as f, useCartStore as g, useWishlistStore as h, useToast as i, __nuxt_component_1 as j, useAPI as k, useNuxtApp as l, defineNuxtRouteMiddleware as m, navigateTo as n, useAuthStore as o, useState as p, __nuxt_component_2$1 as q, tv as t, useHead as u };
