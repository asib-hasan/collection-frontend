  ...
  <div class="min-h-screen bg-background flex items-center justify-center px-4 py-8 md:py-16 relative overflow-hidden">
    ...
    <div class="w-full max-w-md relative z-10">
      ...
      <div class="bg-white/80 backdrop-blur-xl border border-white shadow-2xl rounded-3xl p-6 sm:p-8 md:p-10">
        <h1 class="text-2xl font-black text-center text-gray-900 mb-1">Update Password</h1>
        ...
        <UForm ref="login-form" :schema="schema" :state="state" @submit="onSubmit" class="space-y-6">
          <UFormField label="New Password" name="new_password" size="lg">
            <UInput
          ...
          <UFormField label="Confirm Password" name="new_password_confirmation" size="lg">
            <UInput
            ...
            >
              <template #trailing>
                <UButton
                  color="neutral"
                  variant="ghost"
                  :icon="confirm_show ? 'mdi:eye-off-outline' : 'mdi:eye-outline'"
...
const schema = object({
  new_password: string().required("New Password Field is Required"),
  new_password_confirmation: string().required("Confirm Password Field is Required"),
});
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  formRef.value?.clear();
  try {
    const response: any = await $api("/customer/reset-password", {
      method: "POST",
      body: event.data,
    });
    ...
    toast.add({
      title: "Success",
      description: response.message || "Password Updateted Successfully",
    });
